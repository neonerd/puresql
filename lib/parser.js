...
function parseQuery (parameters, sql, adapter) {
  ...
  console.log('parameters options', parsedParameters.parametersOptions)
  ...
  if (parameters['?']) {
    for (let parameter of parameters['?']) {
      sql = sql.replace(':?', escapeParameter(parameter, {strToReplace: ':?'}, adapter))
    }
  }
